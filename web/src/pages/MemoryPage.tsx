...
export default function MemoryPage() {
  ...
  return (
    <div className="fluid-page memory-workspace">
      <Card
        title="记忆"
...
function ProfilePanel() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setLoadError((e as Error).message || '画像暂时无法加载')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      {loading && !profile ? (
        <WorkspaceState kind="loading" title="正在加载记忆画像" />
      ) : loadError ? (
        <WorkspaceState kind="error" title="记忆画像加载失败" description={loadError} onRetry={load} />
      ) : !profile || profile.total === 0 ? (
        <WorkspaceState kind="empty" title="还没有记忆" description="主动记住一些事，或在对话中聊聊你自己，我会自动记住。" />
      ) : (
        <>
                ...
                {group.entities.map((ent) => (
                  ...
                  >
                    ...
                    {ent.relations.length > 0 && (
                      <div className="memory-relations" style={{ marginTop: 8, paddingLeft: 8 }}>
          ...
          {hits.map((h) => (
            <Card key={h.id} size="small" styles={{ body: { padding: 16 } }}>
              ...
              {h.relations.length > 0 && (
                <div className="memory-relations" style={{ paddingLeft: 8 }}>
