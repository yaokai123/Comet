...
export default function AgentTaskPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(error instanceof Error ? error.message : '定时任务加载失败')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <div className="fluid-page operations-page task-workspace">
      ...
      {loading ? (
        <WorkspaceState kind="loading" title="正在加载定时任务" />
      ) : loadError ? (
        <WorkspaceState kind="error" title="定时任务加载失败" description={loadError} onRetry={load} />
      ) : tasks.length === 0 ? (
        <WorkspaceState
          kind="empty"
          title="还没有定时任务"
          description="新建一个任务，让 AI 按计划持续跟进重要主题。"
          action={<Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>新建任务</Button>}
        />
      ) : (
      <div className="task-list">
        {tasks.map((t) => (
          <Card key={t.id} className="task-card" styles={{ body: { padding: 16 } }}>
