...
export default function ResearchPage() {
  ...
  const startResearch = async () => {
    ...
    setCurrentId(null)
    try {
      await streamResearch(t, buildHandlers(), ac.signal, null, new URLSearchParams(window.location.search).get('project'))
    } catch (err) {
      if ((err as Error)?.name === 'AbortError') return
      const errorMessage = (err as Error)?.message || '研究流连接中断'
  ...
  const stopResearch = () => {
    ...
    setRunning(false)
    setStreamError('研究已停止，当前计划、来源和已撰写内容已保留。')
  }
  ...
  return (
    <div className="fluid-page operations-page research-page" style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, width: '100%' }}>
        ...
        {(running || showReport) && (
          <div ref={bodyRef} className="research-body">
              ...
              <div className="research-progress">
                ...
                {running && (
                  <div className="research-progress-actions">
                    <Button size="small" onClick={stopResearch}>停止研究</Button>
                  </div>
                )}
                {feedOpen && (
            ...
            {streamError && (
              <div className="research-stream-notice" role="status">
                <div>
                  <strong>研究未完整结束</strong>
                  <span>{streamError}</span>
                </div>
                <Space size="small" wrap>
                  {running && <Button onClick={stopResearch}>停止</Button>}
                  {!running && <Button type="primary" onClick={startResearch}>重新开始</Button>}
                  <Button type="text" onClick={() => setStreamError(null)}>关闭</Button>
                </Space>
              </div>
            )}
            ...
            {showReport && (
              ...
              >
                <div className="research-print">
                  ...
                  <MarkdownMessage content={displayMd} className="long-form-content" />
                </div>
