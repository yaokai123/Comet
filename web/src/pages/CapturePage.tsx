...
export default function CapturePage() {
  ...
  return <div className="fluid-page capture-workspace">
    ...
    <Tabs items={[{ key: 'file', label: '上传文件', children: <Dragger accept=".pdf,.docx,.xlsx,.xlsm,.xls,.md,.markdown,.txt,.html,.htm" multiple showUploadList={false} beforeUpload={captureFile} className="capture-dragger"><p className="ant-upload-drag-icon"><InboxOutlined /></p><p className="ant-upload-text">拖入文件，或点击选择文件</p><p className="ant-upload-hint">支持 PDF、Word、Excel、Markdown、TXT 和 HTML，单个文件最大 50MB</p></Dragger> }, { key: 'url', label: '保存网页', children: <div className="capture-url"><p>粘贴公开网页链接，Comet 会提取正文并加入当前知识空间。</p><div><input aria-label="网页链接" value={url} onChange={(event) => setUrl(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && void captureUrl()} placeholder="https://..." /><Button type="primary" icon={<LinkOutlined />} onClick={() => void captureUrl()}>保存网页</Button></div></div> }]} />
