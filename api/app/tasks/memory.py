...
async def _extract(session: AsyncSession, mem_uuid: uuid.UUID) -> None:
    ...
    if not memory:
        ...
        return
    if memory.status == MEMORY_STATUS_DONE:
        logger.info("Memory already completed; skipping duplicate task: %s", mem_uuid)
        return
    async with redis_task_lock(f"task-lock:memory-extract:{mem_uuid}") as acquired:
        if acquired:
            await _extract_locked(session, mem_uuid, memory)


async def _extract_locked(session: AsyncSession, mem_uuid: uuid.UUID, memory) -> None:
    repo = MemoryRepository(session)
    try:
