...
class DocumentService:
    ...
    async def _dispatch_parse(self, document_id: uuid.UUID) -> None:
        """Persist an outbox row only; the reconciler owns Celery dispatch.

        This avoids the DB-commit / broker-publish dual-write gap.
        """
        doc = await self.repo.get_by_id(document_id)
        if doc:
            job = DocumentIndexJob(document_id=document_id, generation=doc.generation)
    ...
    async def upload(
        ...
        await self.repo.create(doc, commit=False)
    ...
    async def upload_from_path(
        ...
        """Create a document from a request-spooled file, keeping request memory bounded."""
        ext = Path(file_name).suffix.lower()
        if ext not in SUPPORTED_EXTS:
            raise BizError(f"不支持的文件类型: {ext}", code=3001)
        ...
        doc_id = uuid.uuid4()
        file_key = build_file_key(str(user_id), "documents", str(doc_id), ext)
        await get_storage().save_from_path(file_key, source_path)
        doc = Document(
            id=doc_id,
            user_id=user_id,
            kb_id=resolved_kb,
        ...
        await self._dispatch_parse(doc_id)
        await self.session.commit()
        logger.info("文档分块上传: user=%s id=%s name=%s", user_id, doc_id, file_name)
        return doc
