...
class AuthService:
    ...
    def issue_tokens(self, user: User) -> tuple[str, str]:
        sub = str(user.id)
        return (
            create_access_token(sub, user.token_version),
            create_refresh_token(sub, user.token_version),
        )
    ...
    async def logout(self, user: User) -> None:
        await self.repo.revoke_tokens(user)
        logger.info("用户令牌已撤销: id=%s", user.id)
