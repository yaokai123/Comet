...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        app.state.readiness["postgres"] = await postgres.ping()
        if not app.state.readiness["postgres"]:
            raise RuntimeError("PostgreSQL is unavailable after migration")
    except Exception as e:
        raise RuntimeError("Database migration failed; refusing to start") from e
    ...
    try:
        await ensure_index()
        app.state.readiness["elasticsearch"] = await elastic.ping()
    except Exception as e:
        logger.warning("ES 索引初始化失败（稍后可重试）: %s", e)
    # 启动：初始化记忆图谱 schema（约束 + 向量/全文索引）
    if settings.is_production and not app.state.readiness["elasticsearch"]:
        raise RuntimeError("Elasticsearch is not ready; refusing to start")
    from app.core.memory.graph_schema import ensure_graph_schema

    try:
        await ensure_graph_schema()
        app.state.readiness["neo4j"] = await neo4j.ping()
    except Exception as e:
        logger.warning("记忆图谱 schema 初始化失败（稍后可重试）: %s", e)
    # 启动:Agent Tracing 异步落库器(V0.0.5 ③)
    if settings.is_production and not app.state.readiness["neo4j"]:
        raise RuntimeError("Neo4j is not ready; refusing to start")
    app.state.readiness["redis"] = await redis.ping()
    if settings.is_production and not app.state.readiness["redis"]:
        raise RuntimeError("Redis is not ready; refusing to start")
