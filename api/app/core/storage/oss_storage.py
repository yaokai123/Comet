...
class OssStorage(StorageBackend):
    ...
    async def save_from_path(self, file_key: str, source_path: Path) -> str:
        await asyncio.to_thread(self.bucket.put_object_from_file, file_key, str(source_path))
        logger.info("OSS 保存暂存文件: %s", file_key)
        return file_key
