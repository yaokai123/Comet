...
class Settings(BaseSettings):
    ...
    def validate_runtime(self) -> None:
        ...
        failures: list[str] = []
        if self.app_debug:
            failures.append("APP_DEBUG must be false in production")
        if self.jwt_secret == "change-me-in-production" or len(self.jwt_secret) < 32:
            failures.append("JWT_SECRET must be a unique value of at least 32 characters")
        try:
            Fernet(self.fernet_key.encode())
        except Exception:  # noqa: BLE001
            failures.append("FERNET_KEY must be a valid Fernet key")
        if self.postgres_password in {"", "comet"}:
            failures.append("POSTGRES_PASSWORD must not use the development default")
        if self.neo4j_password in {"", "cometneo4j"}:
            failures.append("NEO4J_PASSWORD must not use the development default")
        if not self.cors_origin_list or "*" in self.cors_origin_list:
            failures.append("CORS_ORIGINS must contain explicit production origins")
        if failures:
